...
class TetrisAgent:
    ...
    def compute_features(self, board: np.ndarray) -> torch.Tensor:
        """Compute board features from a 2D board array (values 0 or 255)."""
        height, width = board.shape
        holes = self.calculate_holes(board)
        aggregate_height, bumpiness, max_height = self.calculate_height(board)
        return torch.tensor([
            holes / (height * width),
            bumpiness / (height * width / 2),
            aggregate_height / (height * width),
            max_height / height
        ], dtype=torch.float32)
    ...
    def select_action(self, state: np.ndarray, epsilon: float = 0.01):
        ...
        with torch.no_grad():
            board = state[-1]  # most recent frame from frame stack
            features = self.compute_features(board).unsqueeze(0).to(self.device)
            state_t = torch.as_tensor(state, dtype=torch.float32).unsqueeze(0).to(self.device)

            q_values = self.model(state_t, features, normalize=True)
    ...
    def update(self, batch_size: int):
        ...
        if isinstance(self.buffer, PrioritizedReplayBuffer):
            ...
            states, actions, rewards, next_states, dones, weights, indices, features, next_features = self.buffer.sample(batch_size, beta=self.beta)
        else:
            states, actions, rewards, next_states, dones, features, next_features = self.buffer.sample(batch_size)
        ...
        next_features = next_features.float().to(self.device)

        with torch.no_grad():
            next_actions = self.model(next_states, next_features, normalize=True).argmax(1, keepdim=True)
            max_next_q = self.target(next_states, next_features, normalize=True).gather(1, next_actions)

            dones = dones.unsqueeze(-1)
            targets = rewards.unsqueeze(-1) + (1 - dones) * (self.gamma ** self.n_step) * max_next_q
    ...
    def push(self, state, action, reward, next_state, done, features, next_features):
        self.buffer.push(state, action, reward, next_state, done, features, next_features)

    def calculate_holes(self, board: np.ndarray):
        holes = 0
        for col in range(board.shape[1]):
            for row in range(board.shape[0]-1):
                if board[row, col] == 255 and board[row+1, col] == 0 and self._has_path_to_bottom(board, row, col):
                    holes += board.shape[0] - row
        return holes

    def _has_path_to_bottom(self, board: np.ndarray, start_row: int, start_column: int):
        height, width = board.shape
        visited = set()
    ...
    def calculate_height(self, board: np.ndarray):
        ...
        for col in range(board.shape[1]):
            ...
            if self._has_path_to_bottom(board, highest_row, col):
